...
class _MockFusedNorm:
    ...
    def __call__(self, x, residual=None):
        if residual is None:
            return x.clone()
        # Return outputs WITHOUT grad (mimics no_grad in _layer_forward)
...
class _PatchToyDims:
    """Context manager that patches bt_lora_trainer constants to toy dims."""
...
class TestTPSharding(unittest.TestCase):
    ...
    def test_tp2_o_proj_a_halved_along_input(self):
        ...
        for key, t in o_a[:4]:
            ...
            self._shard(t, 1, 2, "lora_A", "o_proj")
...
class TestSyncToVLLM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        attn = types.SimpleNamespace(
            ...
            o_proj=types.SimpleNamespace(weight=torch.zeros(TOY_H, q_sz, device=dev)),
        )
        ...
        self.trainer = types.SimpleNamespace(
            ...
            _lora={
                0: {
                    "q_proj.lora_A": torch.randn(
                        TOY_RANK, TOY_H, device=dev
                    ).requires_grad_(True),
                    "q_proj.lora_B": torch.randn(
                        q_sz, TOY_RANK, device=dev
                    ).requires_grad_(True),
                    "v_proj.lora_A": torch.randn(
                        TOY_RANK, TOY_H, device=dev
                    ).requires_grad_(True),
                    "v_proj.lora_B": torch.randn(
                        kv_sz, TOY_RANK, device=dev
                    ).requires_grad_(True),
                    "o_proj.lora_A": torch.randn(
                        TOY_RANK, q_sz, device=dev
                    ).requires_grad_(True),
                    "o_proj.lora_B": torch.randn(
                        TOY_H, TOY_RANK, device=dev
                    ).requires_grad_(True),
                }
            },
        )
    ...
    def test_q_proj_delta_applied_correctly(self):
        ...
        torch.testing.assert_close(
            w.to(torch.bfloat16), expected, msg="q_proj delta not applied correctly"
        )
    ...
    def test_o_proj_delta_applied_correctly(self):
        ...
        torch.testing.assert_close(
            w.to(torch.bfloat16), expected, msg="o_proj delta not applied correctly"
        )
    ...
    def test_second_sync_reflects_updated_lora(self):
        """After an optimizer step updates LoRA A, the second sync applies it."""
        self._call_sync()
...
class TestGradientFlow(unittest.TestCase):
        ...
        for proj in ("v_proj", "o_proj"):
            for ab in ("lora_A", "lora_B"):
                ...
                self.assertIsNotNone(
                    param.grad, f"{key} in last layer has no gradient after backward"
                )
                self.assertGreater(
    ...
    def test_optimizer_step_updates_weights(self):
        ...
        self.assertFalse(
            torch.equal(A_v_init, A_v_after),
...
class TestSubOpBuilders(unittest.TestCase):
    ...
    def test_fwd_subops_count(self):
        ...
        with _PatchToyDims():
            trainer = _make_bare_trainer(
                _make_mock_model(DEVICE), self.adapter_path, device_idx=self._dev_idx()
            )
    ...
    def test_fwd_subops_run_all_layers_and_store_result(self):
        ...
        self.assertIn(
            "hidden_last", trainer._fwd, "hidden_last not stored after all fwd sub-ops"
        )
        self.assertIn("labels", trainer._fwd, "labels not stored after all fwd sub-ops")
        self.assertIn(
            "layers", trainer._fwd, "layers dict not stored after all fwd sub-ops"
        )
        self.assertEqual(
            len(trainer._fwd["layers"]),
            TOY_LAYERS,
            f"Expected {TOY_LAYERS} layer entries, got {len(trainer._fwd['layers'])}",
        )
        ...
        for key in ("x_norm", "attn_out", "x2"):
            self.assertIn(
                key, last, f"'{key}' missing from layer {TOY_LAYERS - 1} activations"
            )

    def test_bwd_subops_count(self):
        """build_bwd_subops must return (N_chunks+4+4)*N + 2 sub-ops.

        With _bwd_passthrough_chunks=1 (set in _make_bare_trainer):
          1 passthrough + 4 attn bwd + 4 LoRA (Q/K/V/O) = 9 per layer,
          plus lm_head and optimizer.
        """
        with _PatchToyDims():
            trainer = _make_bare_trainer(
                _make_mock_model(DEVICE), self.adapter_path, device_idx=self._dev_idx()
        ...
        expected = (n_chunks + 4 + 4) * TOY_LAYERS + 2
    ...
    def test_subop_qlora_grads_nonzero_after_sync(self):
        """After one optimizer step + _sync_to_vllm, Q LoRA grads must be non-zero.
        ...
        with _PatchToyDims():
            model = _make_mock_model(DEVICE)
            trainer = _make_bare_trainer(
                model, self.adapter_path, device_idx=self._dev_idx()
            )
                ...
                for ab in ("lora_A", "lora_B"):
                    ...
                    if grad is not None:
                        self.assertGreater(
                            ...
                            "(possible _prev_delta bug in _attn_o_proj_bwd"
                            "/_attn_qkv_recompute)",
                        )
