...
from test_bt_lora_tp_correctness import Q_SZ, _build_trainer
...
def _check(results: dict) -> bool:
    ...
    for tag, r in results.items():
        ...
        loss_match = (
            abs(r["tp2"][0]["loss"] - r["ref_loss"]) < 1e-4
            and abs(r["tp2"][1]["loss"] - r["ref_loss"]) < 1e-4
        )
        ...
        if not any_nonzero_ref:
            ...
            if not (all_match and loss_match):
                ok = False
                print(
                    "  FAIL: expected all layers' grads + loss to match the "
...
def main() -> None:
    ...
    with _PatchToyDims():
        ...
        qkv_full = [
            (torch.randn(Q_SZ + 2 * (TOY_KV * TOY_HD), TOY_H, generator=g) * SCALE).to(
                device
            )
        ...
        w13_full = (torch.randn(E, 2 * D_INTER, TOY_H, generator=g) * MOE_SCALE).to(
            device
        )
        ...
        # Vocab-sharded embedding and lm_head for TP=2 (production layout —
        # VocabParallelEmbedding shards the same way as ParallelLMHead).
        # Each rank holds tokens [rank*v_half, (rank+1)*v_half); the exchange
        # sums the partial embeddings back to the full embedding row.
        # The TP=1 reference keeps both full tables.
        v_half = TOY_VOCAB // 2
        embed_w_local = embed_w[rank * v_half : (rank + 1) * v_half].contiguous()
        lm_w_local = lm_w[rank * v_half : (rank + 1) * v_half].contiguous()

        adapter_path = os.path.join(
            tempfile.gettempdir(), "bt_lora_moe_bwd_test_adapter.safetensors"
        )
        if rank == 0:
        ...
        for tp_correct in (True, False):
            ...
            if rank == 0:
                ...
                results[tag] = {"ref_loss": loss1, "ref_grads": grads1, "tp2": gathered}
            dist.barrier()
