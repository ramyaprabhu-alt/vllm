#!/bin/bash
# Bubble characterisation on 2x A100-SXM4-80GB (SM80).
# Uses real Qwen3-30B-A3B weights.  Triton MoE backend (no FlashInfer cubin on SM80).
set -euo pipefail
...
PORT=8000
LOG="$SCRIPTS_DIR/run_a100_bubble_measure.log"
...
# A100-specific: Triton JIT backend, no FlashInfer autotune
COMMON_FLAGS=(
...
log "=== A100 Bubble Measurement (real weights, Triton MoE, fixed profiling) ==="
log "Model: $MODEL"
log "Hardware: 2x A100-SXM4-80GB (SM80)"
log ""
...
log "  running mixed_bubble_measure.py (4 prompt types x 3 token lengths x 3 reps) ..."
