#!/bin/bash
# Group C rerun: LLMStation first, then vLLM + Bubble Tea
# 9 configs × 3 systems = 27 runs

set -euo pipefail

PYTHON=".venv/bin/python"
LOG="compare_results/group_c_rerun_progress.log"
...
RATES=(0.4 1.0 4.0)
# LLMStation first, then vllm, then bubble_tea
SYSTEMS=(llmstation vllm bubble_tea)
...
for dist in "${DISTS[@]}"; do
  for rate in "${RATES[@]}"; do
    ...
    # Clean stale results from previous failed runs
    rm -f "$result_dir"/{vllm,llmstation,bubble_tea}.json
    rm -f "$result_dir"/{vllm,llmstation,bubble_tea}_{bench,server}.log
...
log "=== Group C rerun complete ($done_count/$total runs) ==="
